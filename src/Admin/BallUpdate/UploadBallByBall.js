...
export default function UploadBallByBall() {
  ...
  const [selectedKey, setSelectedKey] = useState(
    "8c2f175bb9msh42d2c4435c1685cp15de67jsnf4fa85f71f8a"
  );
...
//   console.log(match_id)
  ...
  useEffect(() => {
    const fetchMatchData = async () => {
      try {
        ...
        const response = await fetch(infoAPI.toString(),options);
  ...
  async function CloseOver() {
    try {
      let Confirmation = window.confirm(
        "Are you sure to End Over " + openOvers.over_number
      );
      if (!Confirmation) {
  ...
  async function SwitchInnings() {
    try {
      let Confirmation = window.confirm(
        "Are you sure to Switch Innings" + openOvers.innings
      );
      if (!Confirmation) {
        throw new Error("Canceled");
      }
          ...
          <div className="ball_forms_con">
            <div className="ball_forms">
              ...
              <input
                ...
                name="over_breakdown"
              />
              <button className="add_runs" onClick={add_overBreakDown}>
                Add
              </button>
              <label>Runs:</label>
              <input
            ...
            <div className="balls_from_api">
              ...
              >
                <option value="8c2f175bb9msh42d2c4435c1685cp15de67jsnf4fa85f71f8a">
                  Key 1
                </option>
                <option value="d8a433c819msh5f6984e6e8d52d2p10084fjsn90afec517a0a">
                  Key 2
                </option>
                <option value="5beed19ad2mshfedc45ca698b32ep1e1644jsnf4cb628a6065">
                  Key 3
                </option>
                <option value="7bd5c8ca5amsh8d14f218e111023p1b6fcejsn7a1117337a5f">
                  Key 4
                </option>
              </select>

              <textarea value={apiOversData} readOnly></textarea>
              <button onClick={fetchOversFromAPi}>Fetch Overs</button>
            </div>
            <div className="set_open_overs">
              <button
                onClick={CloseOver}
              >{`Close Over ${openOvers.over_number}`}</button>
              <button
                onClick={SwitchInnings}
              >{`End Innings ${openOvers.innings}`}</button>
